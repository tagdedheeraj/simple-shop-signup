...
import { getFirestoreProducts, getFirestoreProductById } from '../firebase/products';
...
export const getRelatedProducts = async (productId: string, limit: number = 4): Promise<Product[]> => {
  ...
  try {
    ...
    const currentProduct = await getFirestoreProductById(productId);
    ...
    console.error('Error getting related products from Firestore:', error);
    ...
    let relatedProducts = products.filter((p: Product) => 
      p.category === currentProduct.category && p.id !== currentProduct.id
    );
...
export const getTrendingProducts = async (limit: number = 4): Promise<Product[]> => {
  ...
  try {
    ...
    console.error('Error getting trending products from Firestore:', error);
